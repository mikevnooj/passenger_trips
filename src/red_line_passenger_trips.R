...
VMH_Raw_90[
  , seconds_since_midnight := data.table::fifelse(
    Transit_Day == data.table::as.IDate(Time)
    , as.numeric(data.table::as.ITime(Time) - as.ITime(Transit_Day))
    , as.numeric(data.table::as.ITime(Time))+24*60*60
    )
  ][
...
VMH_Raw_90[, AdHocTripNumber := stringr::str_c(
  Inbound_Outbound
...
VMH_Raw_90_no_zero[
  ...
  ggplot2::ggplot(ggplot2::aes(x=Boards)) +
  ggplot2::geom_histogram(binwidth = 1)# +
  ggplot2::aes(stat_bin(binwidth = 1
                         , geom = "text"
                         , ggplot2:aes(label = ..x..)
                         , vjust = -1.5
                         )
...
VMH_90_clean <- VMH_Raw_90[
  ...
  , on = c("Transit_Day", "Vehicle_ID")
    ...
    , on = c("Transit_Day", "Vehicle_ID")
      ...
      , nstops := data.table::uniqueN(Stop_Id)
      , AdHocTripNumber
      ][nstops >= 28 | nstops == 21
        ]
...
VMH_Raw_90[
  ...
  , AdHocTripNumber
    ...
    , data.table::uniqueN(AdHocTripNumber)
    , nstops
    ][order(nstops)
      ]
...
fwrite(valid_dt_xuehao
       , paste0("data//processed//VMH_90_Valid_Invalid//"
                , stringr::str_sub(VMH_StartTime, 0, 6)
                , "_valid_trips.csv"
                )
       )
...
files <- paste0("data//processed//VMH_90_Valid_Invalid//"
                , list.files("data//processed//VMH_90_Valid_Invalid//"
                             , pattern = format(this_month_Avail,"%Y")
                             )
                )
...
read_plus <- function(file){
  fread(file) %>%
    dplyr::mutate(valid_invalid = strsplit(file, "_")[[1]][5])
}
...
# VMH_Raw[,uniqueN(Inbound_Outbound),.(Trip,Transit_Day)][V1 > 1]
# VMH_Raw[Trip == 1545, .N, .(Inbound_Outbound,Transit_Day)][order(Transit_Day)]
# get trips operated ------------------------------------------------------
...
trips_operated_90[
  , service_type := data.table::fcase(
    data.table::as.IDate(lubridate::mdy(Date)) %in% 
      data.table::as.IDate(holidays_saturday@Data)
    , "Saturday"
    , data.table::as.IDate(lubridate::mdy(Date)) %in% 
      data.table::as.IDate(holidays_sunday@Data)
    , "Sunday"
    , weekdays(data.table::as.IDate(lubridate::mdy(Date))) %in% c("Monday"
                                                                  , "Tuesday"
                                                                  , "Wednesday"
                                                                  , "Thursday"
                                                                  , "Friday"
    ...
    , "Weekday"
    , weekdays(data.table::as.IDate(lubridate::mdy(Date))) == "Saturday"
    , "Saturday"
    , weekdays(data.table::as.IDate(lubridate::mdy(Date))) == "Sunday"
    , "Sunday"
    )#end fcase 
  ]
...
trips_operated_90[,n := .N, AdHocTripNumber]
trips_operated_90[n > 1] %>% View()


trips_operated_90[
  , trip_start_hour := data.table::hour(time)
  ][, Transit_Day := data.table::as.IDate(lubridate::mdy(Date))
    ][, trip_start_hour := data.table::fifelse(trip_start_hour == 4
                                               , 5
                                               , trip_start_hour
                                               )
      ]

trips_operated_90[, .N, Transit_Day
                  ][order(Transit_Day)
                    ] %>% View()
...
apc_trips_valid <-
  apc_trips_data[
    !apc_trips_data[Transit_Day <= "2020-01-31" & Boards > 757 | 
                      Transit_Day %between% c("2020-02-01"
                                              , "2020-02-29"
                                              ) & Boards > 1000                                                   
                    ]
    , on = c("AdHocTripNumber")
    ][Transit_Day >= lubridate::floor_date(month_end, "year")
      ][Inbound_Outbound != 9 & Inbound_Outbound != 14
        ][, trip_start_hour := data.table::fifelse(trip_start_hour == 4
                                                   , 5
                                                   , trip_start_hour
                                                   )
...
trips_operated_per_stratum <- trips_operated_90[, .(trips_operated = .N) 
                                                , .(trip_start_hour
                                                    , service_type
                                                    , Inbound_Outbound
                                                    )
...
expanded_operated_trips <- data.table::merge.data.table(
  trips_operated_per_stratum
...
apc_boardings_by_stratum <- apc_trips_valid[
  valid_invalid == "valid" & Transit_Day <= month_end
  , .(boards = sum(Boards)
      , trips = .N
      )
  , .(trip_start_hour
...
expanded_operated_trips <-merge.data.table(
  trips_operated_per_stratum
  , apc_boardings_by_stratum
  , by.x = c("service_type"
             , "trip_start_hour"
...
#get service table
service_table <- trips_operated_90[
  Transit_Day <= month_end
  ][ , .(service_days_in_month = uniqueN(Transit_Day))
     , service_type
     ]

month_summary <- expanded_operated_trips[
  , .(boardings = sum(final_boardings,na.rm = TRUE))
  , service_type
  ][service_table
    , on = "service_type"
...
ytd_month_summary <- expanded_operated_trips[
  , .(boardings = sum(final_boardings
                      , na.rm = TRUE
                      )
      )
  , service_type
...
service_table <- trips_operated_90[
  Transit_Day <= month_end 
  & Transit_Day > prev_month_end
  ][, .(service_days_in_month = data.table::uniqueN(Transit_Day))
    , service_type
    ]

trips_operated_90[,.N,.(Transit_Day,service_type)] %>% View()
...
month_summary <- prev_ytd_summary[
  ytd_month_summary
  , on = "service_type"
          ...
          , boardings = month_boardings
                  ...
                  , average_daily_boardings = 
                    round(boardings/service_days_in_month)
                  )
           ][]
...
trips_operated_90[Transit_Day > prev_month_end,.N,.(Transit_Day,service_type)][order(service_type)]
...
trips_operated_to_send_to_avail <- trips_operated_90[
  Transit_Day > prev_month_end][,.(`Rte stat`
                                   , Cancelled
                                   , Distance
                                   , Duration
                                   , SchType
...
trips_operated_to_send_to_avail[,.(unique(`Internal trp number`))
                                ][V1 %in% DimTrip$PermanentTripNumber
                                  ][V1 %in% trips_operated_90[
                                    , .(unique(`Internal trp number`))
                                    ][V1 %in% DimTrip$TripInternalNumber]$V1
                                    ]
...
DimTrip[TripInternalNumber %in% trips_operated_to_send_to_avail$`Internal trp number`]
